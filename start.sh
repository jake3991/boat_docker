...
sudo docker run --rm -it --net=host --ipc=host --pid=host -v /media/boat/external_dr/jetson_data:/jetson_data -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY="$DISPLAY" --privileged -v /dev/serial:/dev/serial:ro -v /dev/bus/usb:/dev/bus/usb wrc/boat:humble bash
